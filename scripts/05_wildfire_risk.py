...
fuel     = np.clip((ndvi - 0.15) / (0.7 - 0.15), 0, 1)
fuel     = np.where(ndvi == -9999, np.nan, fuel)   # sea/nodata stays out
...
for path, arr, dtype in [("data/kefalonia_wildfire_risk.tif", risk, "float32"),
                         ("data/kefalonia_wildfire_risk_class.tif", klass, "int16")]:
    with rasterio.open(path, "w", driver="COG", height=H, width=W, count=1,
        ...
        dst.write(arr, 1)
print("saved: kefalonia_wildfire_risk.tif + kefalonia_wildfire_risk_class.tif")
