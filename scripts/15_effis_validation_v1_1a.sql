...
SELECT * FROM (
  ...
  UNION ALL
  SELECT 'v1_1a', q.effis_id, q.fire_date, q.area_ha, (q.s).count, (q.s).mean
  FROM (SELECT p.effis_id,p.fire_date,p.area_ha,
               ST_SummaryStats(ST_Union(ST_Clip(r.rast,p.geom)),true) AS s
        FROM argos.effis_perimeters p JOIN argos.wildfire_risk_v1_1a r ON ST_Intersects(r.rast,p.geom)
        GROUP BY p.effis_id,p.fire_date,p.area_ha) q
) z;
...
UNION ALL
SELECT 'v1_1a', (s).mean FROM (SELECT ST_SummaryStats(ST_Union(rast),true) s FROM argos.wildfire_risk_v1_1a) q;
...
SELECT v.effis_id, v.fire_date, v.area_ha, round(v.mean::numeric,1) v1, round(a.mean::numeric,1) v1_1a, round((a.mean-v.mean)::numeric,1) AS d
FROM (SELECT * FROM fmean WHERE t='v1') v JOIN (SELECT * FROM fmean WHERE t='v1_1a') a USING(effis_id)
ORDER BY d ASC;
...
WITH icls AS (
  ...
  UNION ALL
  SELECT 'v1_1a',(vc).value,sum((vc).count) FROM (SELECT ST_ValueCount(rast,1,true) vc FROM argos.wildfire_risk_v1_1a_class) q WHERE (vc).value>=0 GROUP BY 1,2
), bcls AS (
  ...
  UNION ALL
  SELECT 'v1_1a',(vc).value,sum((vc).count)
  FROM (SELECT ST_ValueCount(ST_Union(ST_Clip(c.rast,p.geom)),1,true) vc
        FROM argos.effis_perimeters p JOIN argos.wildfire_risk_v1_1a_class c ON ST_Intersects(c.rast,p.geom) GROUP BY p.effis_id) q
  WHERE (vc).value>=0 GROUP BY 1,2
)
SELECT b.t,b.cls,
